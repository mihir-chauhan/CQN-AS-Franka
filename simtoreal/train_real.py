...
def parse_args():
    ...
    p.add_argument("--home-sequence", type=str, nargs="+", default=None,
                   help="Ordered list of .npy files (7-dof joint configs) "
                        "to execute during reset. Paths are resolved relative "
                        "to the simtoreal/ directory if not absolute. "
                        "Default: partialhome.npy image_45.npy")
    p.add_argument("--task-mode", choices=["reach-grasp", "pick-place"],
                   default="reach-grasp",
                   help="'reach-grasp' (default): gripper stays open, no "
                        "human pause. 'pick-place': after homing, pauses for "
                        "operator to load object, then closes gripper and "
                        "starts the episode.")
...
def main():
    ...
    # Paths relative to simtoreal/ if not absolute, matching existing defaults.
    home_sequence = None
    if args.home_sequence:
        simtoreal_dir = Path(__file__).resolve().parent
        home_sequence = []
        for p in args.home_sequence:
            ...
            home_sequence.append(np.load(str(path)).tolist())
        print(f"[home-sequence] {len(home_sequence)} waypoints from: "
              f"{args.home_sequence}")

    # Task mode → gripper state + human pause.
      ...
      try:
        ...
        if time_step.last():
            ...
            if global_step >= args.eval_every and global_step % args.eval_every < episode_step:
                ...
                # run_eval uses blocking input() (pause_for_human, human_reward).
                _restore_stdin()
                try:
