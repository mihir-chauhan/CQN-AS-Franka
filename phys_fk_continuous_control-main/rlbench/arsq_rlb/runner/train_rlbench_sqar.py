...
import arsq_rlb.env.rlbench_env as rlbench_env
import arsq_rlb.util.utils as utils
from arsq_rlb.util.logger import Logger
from arsq_rlb.util.replay_buffer import ReplayBufferBatch, ReplayBuffer
from arsq_rlb.util.video import TrainVideoRecorder, VideoRecorder
from arsq_rlb.alg.sqar import SQARAgent
...
class Workspace:
    def __init__(self, cfg):
        ...
        agent_target = self.cfg.agent.get("_target_", "")

        if "CQNASAgent" in agent_target:
            # Hydra automatically maps self.cfg.agent values to CQNASAgent's explicit kwargs
            self.agent = hydra.utils.instantiate(
                self.cfg.agent,
                rgb_obs_shape=self.train_env.rgb_observation_spec().shape,
                low_dim_obs_shape=self.train_env.low_dim_observation_spec().shape,
                action_shape=self.train_env.action_spec().shape,
                device=self.device # Passing device explicitly if needed
            )
        else:
            # Fallback to the monolithic config injection for SQARAgent
            self.agent = SQARAgent(
                cfg=self.cfg.agent,
                rgb_obs_shape=self.train_env.rgb_observation_spec().shape,
                low_obs_shape=self.train_env.low_dim_observation_spec().shape,
                action_shape=self.train_env.action_spec().shape,
                use_logger=self.cfg.use_tb or self.cfg.use_wandb,
            )
            # self.agent = SQARAgent(
            #     self.cfg.agent,
            #     self.train_env.rgb_observation_spec().shape,
            #     self.train_env.low_dim_observation_spec().shape,
    ...
    def setup(self):
        ...
        self.replay_buffer = ReplayBufferBatch(
            data_specs,
        ...
        self.demo_replay_buffer = ReplayBufferBatch(
            data_specs,
            self.cfg.use_relabeling,
            True,
            self.cfg.replay_buffer_size,
    ...
    def replay_iter(self):
        if self._replay_iter is None:
            def replay_iter_func():
                while True:
                    yield self.replay_buffer.sample(self.cfg.batch_size)
            def demo_replay_iter_func():
                while True:
                    yield self.demo_replay_buffer.sample(self.cfg.batch_size)
            replay_iter = replay_iter_func()
            demo_replay_iter = demo_replay_iter_func()
    ...
    def train(self):
        # set
        self.replay_buffer.set_online_buffer(self.cfg.online_buffer)
        self.demo_replay_buffer.set_online_buffer(self.cfg.online_buffer)
        ...
        self.demo_replay_buffer.add(time_step)
            ...
            if time_step.last():
                ...
                if metrics is not None:
                    ...
                    with self.logger.log_and_dump_ctx(
                        ...
                        log("demo_buffer_size", len(self.demo_replay_buffer))
                        log("step", self.global_step)
...
@hydra.main(config_path="../cfgs", config_name="config_rlbench_cqnas")
def main(cfg):
    root_dir = Path.cwd()
