...
class CQN:
    def __init__(self, cfg, obs_shape, action_shape):
        ...
        # Physics-Informed Reg Params
        self.use_eikonal = getattr(cfg, 'use_eikonal', False)
        self.use_fk_reg = getattr(cfg, 'use_fk_reg', False)
        self.nu = getattr(cfg, 'nu', 0.01)
        self.kappa = getattr(cfg, 'kappa', 0.1)
        self.num_walks = getattr(cfg, 'num_walks', 10)
        self.fk_weight = getattr(cfg, 'fk_weight', 1.0)
    ...
    def _update_critic(self, obs, action, reward, discount, next_obs, demos):
        ...
        if self.use_eikonal or self.use_fk_reg:
        ...
        # =====================================================================
        # 2. EXTRACT SCALAR Q-VALUES FOR V(s) ANCHOR
        # =====================================================================
        if self.use_eikonal or self.use_fk_reg:
            physics_loss = torch.tensor(0.0, device=self.device)
            
            # Collapse to scalar expected Q values [B, L, D, bins]
            if self.atoms == 1:
                qs_val = qs.squeeze(-1) 
            else:
                qs_val = (q_probs * self.critic.support.expand_as(q_probs)).sum(-1)

            # Marginalize over the implicit softmax policy to get V(s)
            temperature = 1.0
            pi_a = torch.softmax(qs_val / temperature, dim=-1)
            expected_q = (pi_a * qs_val).sum(dim=-1) # [B, L, D]
            v_anchor = expected_q.sum(dim=(1, 2)) # [B]

            # =================================================================
            # --- Exact Autograd Eikonal Regularizer ---
            # =================================================================
            if self.use_eikonal:
                grads = torch.autograd.grad(
                    outputs=v_anchor.sum(),
                    inputs=obs,
                    create_graph=True,
                    retain_graph=True,
                    only_inputs=True,
                ...
                grad_norm = torch.linalg.norm(grads, dim=-1) # [B]
                max_slope = self.kappa / (torch.ones_like(grad_norm) + 1e-6)
                slope_excess = torch.relu(grad_norm - max_slope)
                eikonal_loss = slope_excess.pow(2).mean()
                
                physics_loss += self.fk_weight * eikonal_loss
                metrics["eikonal_loss"] = eikonal_loss.item()
                metrics["avg_grad_norm"] = grad_norm.mean().item()

            # =================================================================
            # --- Stochastic Walk-on-Spheres (FK) Regularizer ---
            # =================================================================
            if self.use_fk_reg:
                B, D_obs = obs.shape
                ...
                flat_obs = (obs.unsqueeze(1) + noise).view(B * self.num_walks, D_obs)
                
                # Expand action to match neighbor paths to ensure proper zoom-in context
                flat_action = action.unsqueeze(1).expand(-1, self.num_walks, -1).reshape(B * self.num_walks, -1)
                
                # Query networks
                if self.atoms == 1:
                    flat_qs, _ = self.critic(flat_obs, flat_action)
                    flat_qs_val = flat_qs.squeeze(-1)
                else:
                    flat_q_probs, _, _, _ = self.critic(flat_obs, flat_action)
                    flat_qs_val = (flat_q_probs * self.critic.support.expand_as(flat_q_probs)).sum(-1)
                    
                # Compute neighbor V(s')
                flat_pi_a = torch.softmax(flat_qs_val / temperature, dim=-1)
                flat_expected_q = (flat_pi_a * flat_qs_val).sum(dim=-1)
                v_neighbors = flat_expected_q.sum(dim=(1, 2)).view(B, self.num_walks)
                
                # Compute gradient estimate
                diff = (v_neighbors - v_anchor.unsqueeze(1).detach()).abs()
                grad_epsilon = torch.linalg.norm(noise, dim=-1) + 1e-6
                grad_estimate = diff / grad_epsilon
                
                # Hinge loss against speed limit
                max_slope = self.kappa / (torch.ones((B,), device=self.device) + 1e-6)
                slope_excess = torch.relu(grad_estimate - max_slope.unsqueeze(1))
                fk_loss = slope_excess.pow(2).mean()
                
                physics_loss += self.fk_weight * fk_loss
                metrics["fk_loss"] = fk_loss.item()
                
            # Add to total loss
            critic_loss = critic_loss + physics_loss
