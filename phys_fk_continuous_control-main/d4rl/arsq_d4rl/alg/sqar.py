...
class SQAR:
    ...
    def _update_step(self, batch, cql_m_q_w):
        ...
        if getattr(self.cfg, 'use_eikonal', False):
        ...
        if getattr(self.cfg, 'use_eikonal', False):
            ...
            current_speed = torch.ones((observations.shape[0],), device=self.device)
            kappa = getattr(self.cfg, 'kappa', 0.1)
            max_slope = kappa / (current_speed + 1e-6)

            slope_excess = torch.relu(grad_norm - max_slope)
            eikonal_loss = slope_excess.pow(2).mean()

            fk_weight = getattr(self.cfg, 'fk_weight', 1.0)
            physics_loss += fk_weight * eikonal_loss
            metrics['eikonal_loss'] = eikonal_loss.item()

        # =====================================================================
        # --- STOCHASTIC FK (Walk-on-Spheres) LOSS ---
        # =====================================================================
        if getattr(self.cfg, 'use_fk_reg', False):
            B, D_obs = observations.shape
            nu = getattr(self.cfg, 'nu', 0.01)
            sigma = (2.0 * nu) ** 0.5
            num_walks = getattr(self.cfg, 'num_walks', 10)
            kappa = getattr(self.cfg, 'kappa', 0.1)
            
            # Perturb the entire flat state vector
            noise = torch.randn(B, num_walks, D_obs, device=self.device) * sigma
            flat_obs = (observations.unsqueeze(1) + noise).view(B * num_walks, D_obs)
            
            # Query forward_value_soft directly
            v1_neighbors = self.qf1.forward_value_soft(flat_obs).view(B, num_walks)
            v2_neighbors = self.qf2.forward_value_soft(flat_obs).view(B, num_walks)
            ...
            grad_estimate = diff / (grad_epsilon * (v_anchor.abs().unsqueeze(1) + 1.0))
            ...
            fk_weight = getattr(self.cfg, 'fk_weight', 0.1)
            physics_loss += fk_weight * fk_loss
            metrics['fk_loss'] = fk_loss.item()
            metrics['diff'] = diff.mean().item()
        ...
        qf_loss = qf1_loss + qf2_loss + physics_loss
