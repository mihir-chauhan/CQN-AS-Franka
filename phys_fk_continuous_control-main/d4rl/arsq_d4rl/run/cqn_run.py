...
import d4rl
from arsq_d4rl.alg.cqn_as import CQN
from arsq_d4rl.run.base_run import print_metrics, get_time_metrics, log, stat_traj
...
class CQNRunner:
    def __init__(self, cfg):
        ...
        self.alg = CQN(cfg.alg, observation_dim, action_dim)
        ...
        for epoch in range(cfg.n_pretrain_epochs):
            ...
            with Timer() as train_timer:
                for train_step in range(cfg.n_train_step_per_epoch_offline):
                    ...
                    train_metrics = self.alg.update(batch, 0)  # cqn
        ...
        for epoch in range(100000):
            ...
            with Timer() as train_timer:
                ...
                for train_step in range(n_train_step_per_epoch):
                    ...
                    train_metrics = self.alg.update(batch, self.buffer_rl.total_steps)  # cqn
...
@hydra.main(config_path="../cfgs", config_name="cqn", version_base=None)
def main(cfg):
    os.chdir(hydra.core.hydra_config.HydraConfig.get().runtime.output_dir)

    runner = CQNRunner(cfg)
